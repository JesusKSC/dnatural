...
export default function RegistrarScreen() {
  ...
  const guardar = () => {
    ...
    // Cable Sheets stub (fire & forget)
    const { movimientos } = useStore.getState();
    syncMovimiento(movimientos[movimientos.length - 1]).catch(() => {});
    ...
    // Show toast after sheet close animation
  ...
  return (
    <View style={styles.screen}>
      ...
      {/* ── Product grid with scrollable header ── */}
      <FlatList
        data={lista}
        numColumns={2}
          ...
          <View>
            ...
            <View style={styles.toggle}>
              <TouchableOpacity
                ...
                activeOpacity={0.8}
              >
                <Text style={[styles.toggleText, esVenta && styles.toggleTextOn]}>
                  🛒 Venta
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
            ...
            >
              {categorias.map(cat => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <Text style={[styles.chipText, catSel === cat && styles.chipTextOn]}>
                    {cat}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <>
              ...
              <View style={styles.sheetTop}>
                <View
                  style={[
                    styles.sheetPh,
                    { backgroundColor: CatBg[selected.categoria] ?? Colors.crema2 },
                  ]}
                >
                  <Text style={styles.sheetPhIcon}>
                    {CatIcon[selected.categoria] ?? '📦'}
                  </Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.sheetNombre} numberOfLines={2}>
                    {selected.nombre}
                  </Text>
                  <Text style={styles.sheetSub}>
                    {esVenta ? 'Registrar venta' : 'Registrar compra (ingreso)'}
                    ...
                    {(
                      ...
                      >
                        ...
                        <Text
                          style={[
                            styles.optText,
                            metodo === opt.value && styles.optTextOn,
                          ]}
                        >
                          {opt.label}
                        </Text>
...
const styles = StyleSheet.create({
  ...
  optBtnOn: {
    ...
    backgroundColor: 'rgba(127,194,65,0.12)',
  },
